...
# including some modifications
	...
	if [ -n "$(git symbolic-ref HEAD 2> /dev/null)" ]; then
		...
		do
		    ...
		    echo -n "Number of lines diff: "
		    git log "${LOGOPTS[@]}" "${END_AND_BEGIN[@]}" --all --numstat --format="%n" --author="$a" | cut -f1 -f2 | awk '{s+=$1-$2} END {print s}'
...
# including some modifications
	...
	if [ -n "$(git symbolic-ref HEAD 2> /dev/null)" ]; then
		...
		do
			echo "=== $a ==="
		    git log "${LOGOPTS[@]}" "${END_AND_BEGIN[@]}" --all --numstat --format="%n" --author="$a" | cut -f1 -f2 | awk '{s+=$1-$2} END {print s " " a}' "a=$a" "$@"
		done
	else
		echo "you're currently not in a git repository"
	fi
